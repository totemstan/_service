...
For example, to create `add`, `cat`, `getinfo`, `dft`, `python`, `R` 
and `opencv` agents that listen for requests on port 3333, replace 
the `agents = {};` with
...
+ the last 5 agents require need$ enabled to include **TOTEM**s 
[matrix minupulator](/github.com/totemstan/man)
+ the last 3 agents demonstrate R-opencv-python support (assummed installed 
on your host).
+ enable needFetch to use **TOTEM**'s [full-featured Fetch](/github.com/totemstan/enums).
